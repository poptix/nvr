...
# Perform object detection using DeepStack AI
def perform_object_detection(image, id):
    url = f"{settings.DEEPSTACK_HOST}/v1/vision/detection"
    headers = {'api-key': settings.DEEPSTACK_API_KEY}
    ...
    if response.status_code == 200:
        ...
        for obj in response_data['predictions']:
            ...
            save_detected_objects_to_database(id, obj['label'], obj['x_min'], obj['x_max'], obj['y_min'], obj['y_max'], obj['confidence'], 'foo')
        return 1
    else:
...
if __name__ == '__main__':
    id = 2  # Replace with the ID of the image you want to process
    main(id)
